...
//! `-display sdl,gl=on,show-cursor=off`. `Spice`/`Dbus` — варианты
//! `DisplayEngine` из §4.3 архитектурного плана, нужны в первую очередь для
//! стриминга в GUI-клиент (`frontend/`, см. его README про риск инпут-лага) —
//! не используются текущим CLI-путём.
...
impl DisplayConfig {
    ...
    /// Конфигурация, соответствующая исходной референсной конфигурации: SDL с
    /// OpenGL, без явного разрешения/DPI/FPS-лимита в самой конфигурации —
    /// поэтому здесь взяты практичные дефолты (1920x1080, 96 DPI, без лимита
    /// FPS), а не значения, прямо вычитанные из неё (она этот вопрос не решает
    /// явно).
