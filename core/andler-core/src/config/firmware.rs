...
pub struct FirmwareConfig {
    /// Включить UEFI/OVMF. Если `false` — QEMU использует встроенный
    /// legacy BIOS, pflash-диски не добавляются, VARS.fd не создаётся.
    /// По умолчанию `true` — текущее поведение ANDLER.
