...
pub enum BackendError {
    ...
    /// Операция требует связи с гипервизор-процессом инстанса (QMP и
    /// т.п.), но сам процесс уже завершился — не то же самое, что `Io`:
    /// это не транзиентная сетевая/протокольная проблема, у которой имеет
    /// смысл предлагать повтор, а факт, что VM больше не существует.
    /// Позволяет вызывающей стороне (daemon/CLI) реагировать иначе —
    /// например, обновить состояние на `Stopped` вместо того чтобы
    /// показывать generic I/O-текст и намекать на повторную попытку.
