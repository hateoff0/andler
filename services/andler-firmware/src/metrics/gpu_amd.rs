//! AMD GPU metrics via `amdgpu` sysfs.
...
/// Finds the first AMD GPU card in sysfs (identified by presence of the
/// `mem_info_vram_used` ABI file, unique to the `amdgpu` driver).
