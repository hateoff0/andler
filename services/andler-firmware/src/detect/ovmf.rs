...
//!
//! Реализует логику из исходной референсной конфигурации (ранее описанной в
...
//!
//! Приоритет соответствует исходной референсной конфигурации: 4M-вариант (`4m.fd` /
...
//!
//! ## Замечание
//! Исходная референсная конфигурация (scripts/start.sh) была сгенерирована/подсказана
//! ИИ и не является эталоном; пути перепроверены вручную и перенесены только
//! проверенные. Список расширен по сравнению с ней: добавлены пути openSUSE и
//! второй вариант написания для Fedora (`OVMF_CODE.fd` без `4m`).
...
/// создаёт его заново из шаблона). Эквивалент `--reset-boot` из исходной
