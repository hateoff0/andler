...
that then talked to a daemon started with a different environment. Each suite runs under a wall-clock watchdog (`E2E_SUITE_TIMEOUT`, default 600s): a hung suite is killed, its remaining processes and daemon.log tail are dumped for diagnosis, and the next suite still starts on a clean daemon — and so does a suite that failed: instances live under `ANDLER_HOME`, not in the store, so the orchestrator clears the instances directory between suites instead of letting one suite's leftovers fail every later suite's ``no instances`` cleanup assertion. A failing suite also dumps the last command's stdout and stderr, which is usually the whole diagnosis. PASS/FAIL lines carry the suite's elapsed time.

Environment knobs: `E2E_LISTEN_ADDR` (daemon address), `E2E_DEEP_GUEST=0` (skip the privileged deep guest tests), `E2E_SUITE_TIMEOUT` (per-suite watchdog), `E2E_ONLY=<substring>` (run only the suites whose name contains it — `docker compose -f docker/e2e/compose.yaml run --rm -e E2E_ONLY=15_connect e2e` turns an 8-minute run into a 20-second one when chasing a single suite). Note that `docker compose run` does not forward the host environment: pass knobs with `-e`.
...
| `34_install_scripts.sh` | `scripts/install.sh` / `scripts/uninstall.sh` against the binaries this image built: the flag contract (`--check-deps`, `--skip-deps`, `--with-optional`, `--dry-run` and the combinations it refuses), the dependency report on a complete host and on one with QEMU hidden from a rebuilt PATH (which must refuse, name the missing binary and carry the install command, installing nothing), the `--dry-run --with-optional` package plan, a local install into a scratch bin dir with the two versions compared and the closing `andler doctor` pass against the harness daemon, the whole release path against a fixture release server that serves the layout the workflow publishes (install of both components and of one from the same archive, the `SHA256SUMS` verification, the refusal of a tampered archive, and the no-catalog refusal when the tag is omitted), the systemd branch appropriate to the host (the container has no user manager, so the service install must refuse and write no unit; a developer host with a live manager asserts the satisfied report instead), and the uninstall paths — keep binaries, refuse an unattended purge or package removal without `--yes`, purge a scratch `ANDLER_HOME` while the harness's own data root survives, then remove the binaries. The scripts refuse to install as root, which the container is, so those assertions run as a scratch user the suite creates and drives through `runuser`; the read-only modes run as root, which is itself part of the contract |
