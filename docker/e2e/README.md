...
| `07_guest.sh` | Guest error paths always; deep tests (privileged container): offline `guest list`/`install`/`remove` against a real Debian rootfs (chroot via qemu-nbd), Android boot-mode switching and ARM translator install through the guestfish appliance (Debian kernel shipped in the image; `/lib/modules` is deliberately not bind-mounted so the appliance boots its own modules) |
