...
expect_out_grep "info reports virtual size" "virtual_size:"
    ...
    cp "$E2E_LAST_OUT" "$WORK/disk-info.json"
    expect_ok "disk info --json reports virtual_size" -- jq -e '.virtual_size == 1073741824' "$WORK/disk-info.json"
    expect_ok "disk info --json reports backing_file null" -- jq -e '.backing_file == null' "$WORK/disk-info.json"
