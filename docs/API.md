...
| `--instances-root <path>` | No | Directory the new instance directory is created in (default: `~/.andler/instances`, honouring `ANDLER_HOME`). Applies to both kinds; in `--file` mode it comes from `instances_root` in the TOML. The daemon's own bookkeeping — `disk.snapshots/` layer paths and the registry scan that re-discovers instances on restart — is keyed to its own root, so an instance placed elsewhere is fully usable while the daemon runs but its snapshot layers land under the daemon's root; to keep instances somewhere else permanently, point the daemon's `ANDLER_HOME` there instead |
...
- **The questions are typed and hinted**: each option is a short label plus a hint for the highlighted entry (`Venus` — `Vulkan 3D — fastest`), and the cursor opens on the recommended answer instead of naming it in the label text. Free-text answers are validated in place, with the error shown under the prompt (`enter an integer between 1 and 65536, e.g. 256`); a name is trimmed before it is used.
