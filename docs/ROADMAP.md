...
- [x] Core: add VM resource limits (CPU pinning, memory overcommit) — `cpu.pinning`/`cpu.affinity` pins the QEMU process via `taskset -c <set>` with a cross-instance overlap gate (`host CPU N is pinned by running instance …`) and a beyond-host-CPU-count validation; `memory.overcommit_mem_lock` mlocks guest RAM (`-overcommit mem-lock=on`); `memory.hugepages` backs RAM with hugetlbfs (`memory-backend-file, mem-path=/dev/hugepages, preallocate=true`). All three are config keys (writable only while stopped), validated against host topology, covered by e2e suites `26_affinity`/`27_overcommit_mem_lock`/`28_hugepages`.
