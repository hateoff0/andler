...
- [ ] Dev-mode daemon restart that preserves VMs — `ANDLERD_DEV_PRESERVE_VMS=1` (or similar): SIGTERM/Ctrl+C takes the adopt path instead of gracefully stopping the VMs, so the daily dev loop (Ctrl+C → rebuild → restart) keeps the guest alive. Phase 0 (foundations) made the default explicit — graceful SIGTERM stops VMs, adopt is a crash-only safety net (R9 decision, `docs/ARCHITECTURE.md`) — but the original motivation of the reconnect work was exactly the dev loop; this flips the behavior under an explicit opt-in. Needs a careful release of `kill_on_drop` on the daemon side of a live `QemuProcess` before exit, so it lands with the supervisor rework rather than as a bolt-on.
