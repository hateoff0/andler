...
- **External guest steps are bounded.** The libguestfs appliance (`ANDLERD_GUESTFS_TIMEOUT_SECS`, default 300 s), `guestmount` (`ANDLERD_GUEST_MOUNT_TIMEOUT_SECS`, default 120 s) and one chroot batch (`ANDLERD_GUEST_COMMAND_TIMEOUT_SECS`, default 900 s) now have wall-clock limits: a wedged appliance or mount used to hang the operation forever with nothing in the log, and now fails with the step, the elapsed bound and the variable that raises it.
...
- **A translator switch spent minutes in staging.** It ran three guestfish operations per payload file, and a host-side move loop asked the appliance `exists()` twice per file — each `exists()` is its own guestfish run, i.e. its own appliance session, so libndk's ~180 files cost ~530 operations plus ~354 sessions. The payload now travels as one gzipped tar that the guest unpacks with a single command, the move is one in-guest `cp -a`, and the old translator is removed by the guest's shell (the previous cleanup resolved the outgoing translator's guest paths against the *host* filesystem, so libndk's `lib/libndk*` and `lib64/libndk*` were skipped and its libraries stayed in the guest). Measured on a real Android instance afterwards: staging is unchanged at ~3m49s, while an appliance session that unpacks the whole 43 MB payload takes ~2 s and uploading the 17 MB archive ~2 s — so the wall clock is **not** in the operation count, the archive, or the appliance, and the next step is instrumenting the mutator's `apply` and the archive build rather than guessing again. `MutatorOp::RunShell` is internal: the proto carries it so the mapping stays total, but the daemon's request path refuses it, because a manifest must not be able to run arbitrary shell in the guest.
