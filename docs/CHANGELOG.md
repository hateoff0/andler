...
- **A translator switch took minutes.** Staging ran three guestfish operations per payload file — roughly 530 round trips to the appliance for libndk's ~180 files — and that was the whole wall clock of `guest install libndk`. The payload now travels as a single gzipped tar that the guest unpacks with one command (the `bin/` mode fix folded into the same command, the host archive removed afterwards), so a switch is a handful of operations instead of hundreds. `MutatorOp::RunShell` is internal: the proto carries it so the mapping stays total, but the daemon's request path refuses it, because a manifest must not be able to run arbitrary shell in the guest.
