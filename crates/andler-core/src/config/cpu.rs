...
//!
//! Источник истины по дефолтным значениям — `scripts/start.sh`
...
pub struct CpuConfig {
    ...
    pub sockets: u32,
    /// Количество потоков на ядро (`-smp threads=N`). В референсном
    /// `start.sh` всегда `1` (SMT не эмулируется отдельно от физических
    /// ядер хоста).
    pub threads: u32,
    /// Привязка виртуальных CPU к конкретным физическим ядрам хоста
    /// (`taskset`-подобное поведение на стороне `andler-qemu`). `None` —
    /// без привязки, планировщик хоста решает сам (поведение `start.sh`
    /// по умолчанию).
...
impl CpuConfig {
    /// Конфигурация, соответствующая `start.sh`: 4 ядра, 1 сокет, 1 поток
