...
- stdout/stderr процесса QEMU перенаправлены в `Stdio::piped()`, но не читаются активно
  — при достаточно большом объёме вывода в stderr процесс может заблокироваться на
  записи (буфер ОС заполнится). Полноценное логирование вывода — отдельная задача.
