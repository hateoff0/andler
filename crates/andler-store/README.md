...
- `store.rs` — **реализовано**. `Store` — репозиторий с одной таблицей
...
- **Реальные миграции схемы** — на сегодня одна таблица с двумя
  JSON-колонками, поэтому `CREATE TABLE IF NOT EXISTS` достаточно.
  Понадобится настоящий миграционный механизм, только если появится
  типизированная (не JSON) колонка для индексации/фильтрации.
...
`store::tests` — 11 тестов на `Store::open_in_memory()`: round-trip
...
теста на вспомогательный парсинг `InstanceId` из строки.
