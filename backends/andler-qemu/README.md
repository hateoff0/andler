...
| Events | `QmpEventReader` | Async QMP events (SHUTDOWN, DEVICE_DELETED, VSERPORT_CHANGED, BLOCK_IO_ERROR, ...) arrive on a dedicated second `-qmp` monitor (`<qmp>.events.sock`); `QmpEventReader` owns it exclusively (never sends commands) and publishes to a broadcast; the daemon relays them as `DaemonEvent::Qmp` (`andler events`) |
