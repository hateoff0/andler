...
Wizard **полностью на английском** — все вопросы, help-сообщения, сводка и текст ошибок. Это касается только самого интерактивного интерфейса; PLAN.md/WIZARD.md как проектная документация остаются на русском.

### Флаг пропуска

`--quick` — пропуск wizard, все дефолты (авто-детект + значения по умолчанию, без единого вопроса). Требует `--kind`: `andler create --kind linux --name my-vm --quick`. Без `--kind` — ошибка ("--quick requires --kind to specify VM type"). Вместе с `--file` не сочетается ("--quick and --file are mutually exclusive"). OVMF не найден при `--quick --kind android` — жёсткая ошибка (Android требует UEFI); при `--kind linux` — предупреждение и продолжение с Legacy BIOS.

### Два уровня вопросов — выбор внутри wizard, не флаг

Полный список из 10+ категорий настроек (диск, ISO, UEFI, GPU, дисплей, звук, ввод, CPU, память, Android-опции) при показе **всех** вопросов подряд каждому новому пользователю превращает wizard в долгую и утомительную анкету — это противоречит цели "пользователь не парится". Поэтому wizard разделён на два уровня, и выбор между ними — **первый вопрос самого wizard**, а не отдельный CLI-флаг (`--advanced` не существует, удалён из `Command::Create`):
...
Advanced — full control over GPU, GPU memory, resolution, fullscreen,
audio, clipboard, input, CPU, memory, and more.
```

- **Basic** — спрашивает только тип (linux/android), имя, путь к ISO/base-image, версию Android (если применимо), размер диска (дефолт 256 GiB, Enter принимает). Всё остальное (GPU, GPU memory, дисплей, звук, clipboard, ввод, CPU, память, ISO-bus, compact-on-shutdown, ARM-транслятор, root/gapps/microg) берётся из `HardwareDefaults` (см. ниже) и фиксированных дефолтов, без отдельных вопросов.
- **Advanced** — задаёт полный список вопросов по всем категориям ниже, каждый предзаполнен значением из `HardwareDefaults`, где применимо (Enter принимает предложенное).

Это явное архитектурное решение: без разделения на уровни wizard рискует стать длиннее, чем просто передача всех флагов через `--file`/CLI-режим, что обесценивает саму идею "простого" пути для новичка.

### Централизованный авто-детект — `HardwareDefaults`

Всё авто-определение хоста (GPU-вендор и рендер-бэкенд, дисплейный движок, поддержка Venus, аудио-сервер, ARM-транслятор, доступность `passt`, OVMF CODE/VARS) вынесено из wizard в `services/andler-firmware` — единая точка, а не разбросанные проверки внутри CLI. Wizard вызывает ровно одну функцию в самом начале запуска и дальше только читает результат:

```rust
// services/andler-firmware/src/detect/mod.rs
pub struct HardwareDefaults {
    pub ovmf: Result<DetectedOvmf, FirmwareError>,
    pub gpu_render: RenderBackend,             // Venus/VirGL/CPU
    pub display_engine: DisplayEngine,         // SDL/GTK
    pub audio_server: AudioServer,             // PipeWire/PulseAudio/None
    pub arm_translator: Option<ArmTranslator>, // Libndk/Libhoudini/None
...
Ничего не кэшируется — каждый вызов `detect_all()` заново опрашивает хост (sysfs/lspci/`/proc/cpuinfo`/сокеты сессии), так что результат всегда отражает текущее состояние. Отдельные `detect_*`-функции (`detect_gpu_defaults`, `detect_arm_translator`, `detect_audio_server`, `detect_passt_available`, плюс существовавший ранее OVMF-детект) — `pub(crate)` внутри `andler-firmware`; наружу торчит только `detect_all()` и сам тип `HardwareDefaults`, чтобы "что и в каком порядке детектится" было определено ровно в одном месте, а не размазано по wizard и демону.

### Порядок вопросов (Advanced-режим)

1. **Режим конфигурации** — Basic / Advanced (только в самом начале, не повторяется)
2. **Тип и имя** — тип (linux/android), имя VM
3. **Источник** — путь к ISO (Linux) или base-image (Android); пусто для Linux = нет ISO, VM запустится с существующим диском
4. **Версия Android** *(только для `--kind android`)*
5. **Диск** — размер (дефолт 256 GiB, Enter принимает)
6. **ISO-bus** *(только если ISO указан на шаге 3, Linux)* — `virtio-scsi` (быстрее) / `ide` (совместимее), рекомендованный вариант зависит от распознанного имени ISO — см. раздел "Монтирование ISO / CD-ROM"
7. **Compact on shutdown** — сжимать ли образ диска при выключении VM
8. **GPU** — рендер (Venus/VirGL/CPU), дефолт из `HardwareDefaults.gpu_render`; если хост не соответствует минимальным требованиям Venus (`venus_supported = false`) — Venus не предлагается как дефолт, см. раздел GPU
9. **GPU memory** — host memory для рендера, MiB
10. **Разрешение дисплея** — `ШИРИНАxВЫСОТА`, дефолт зависит от `HardwareDefaults.display_engine`
11. **Fullscreen** — да/нет
12. **Звук** — backend, дефолт из `HardwareDefaults.audio_server` (PipeWire/PulseAudio/None) — см. раздел Звук
13. **Clipboard sharing** — да/нет
14. **Ввод** — режим указателя (tablet/mouse/usb)
15. **CPU** — количество ядер
16. **Память** — объём, GiB
17. *(только для `--kind android`)* **GApps** — да/нет
18. *(только для `--kind android`, и только если GApps выключен)* **MicroG** — да/нет
19. *(только для `--kind android`)* **ARM-транслятор** — `none`/`libndk`/`libhoudini`, дефолт из `HardwareDefaults.arm_translator` (по CPU-вендору хоста — AMD → libndk, Intel → libhoudini)
20. *(только для `--kind android`)* **Root-режим** — `none`/`magisk`, путь к Magisk-бинарям при выборе `magisk`
21. **Итоговая сводка** (см. ниже) — финальное подтверждение перед фактическим созданием

Шаги 6–20 в Basic-режиме пропускаются полностью — задаются только шаги 1, 2, 3, 4 (если применимо), 5.

Сетевой режим (NAT/isolated/bridge) и лимит FPS вопросами wizard **не являются** — сеть всегда NAT через `passt` (фоллбэк на `slirp`, если `passt` не установлен — определяется `HardwareDefaults.passt_available`) без отдельного вопроса; выбор режима сети и лимит FPS вынесены в "Планируемые функции" (см. WIZARD.md) и в текущей реализации в wizard не входят ни в Basic, ни в Advanced.
...
Пример (ISO-bus, тот же шаг, что и раньше, в новом интерфейсе; вопрос и help-текст — на английском, как весь wizard):
...
Faster mounting and reading of install media. Works well for modern Linux
distributions (initrd almost always supports virtio-scsi). If the install
doesn't boot, switch to ide below.
...
│  Type:              Linux VM
│  Name:              my-vm
│  ISO:               cachyos-desktop-linux.iso
│  CD-ROM bus:        VirtioScsi (auto-detected)
│  Disk:              ~/.local/share/andler/instances/<id>/my-vm-disk.qcow2 (256 GiB, qcow2)
│  GPU:                Venus (auto-detected)
│  Display:           SDL, 1920x1080 (auto-detected)
...
Это особенно важно для Basic-режима — пользователь не отвечал на большинство вопросов явно, и без сводки не увидит, какие именно дефолты были применены, до момента, когда VM уже создана. Значения, взятые из дефолтов/авто-детекта, а не введённые явно, помечены `(default)`/`(auto-detected)`. "Modify advanced settings" возвращает к Advanced-вопросам с уже введёнными/детектированными значениями как текущими (предзаполненными), не начинает заново с нуля.
...
Зафиксировано, что wizard — третий путь создания VM, не замена существующих TOML/CLI-режимов. Добавлено разделение на Basic/Advanced — без этого wizard со всеми ~20 категориями вопросов для каждого нового пользователя был бы длиннее, чем просто передача флагов напрямую. Добавлен шаг ISO-bus в порядок вопросов (отвечает на вопрос: да, дефолт показывается в скобках и сразу переопределяем тем же способом, что и остальные вопросы). Добавлены Android-специфичные вопросы (gapps/microg/ARM-транслятор/root), которых не было в исходном плане, хотя они уже существуют как флаги в реальном CLI. Добавлен обязательный финальный шаг "Итоговая сводка" — без него пользователь, прошедший Basic-режим, не видит, какие дефолты были к нему применены, до момента, когда VM уже создана. Интерфейс взаимодействия переработан с "ввод номера/текста руками" на стрелки+Enter+Space (как у `docker`/`apt`/большинства современных интерактивных CLI), с конкретным выбором библиотеки (`inquire`) и обработкой `NotTTY`-случая, которого исходная версия плана не учитывала вовсе.

**Обновление после реализации (v2):** `--advanced` как CLI-флаг убран — выбор Basic/Advanced стал первым вопросом самого wizard, а не отдельным флагом (см. "Два уровня вопросов" выше); `andler create --advanced` больше не существует. Весь текст интерактивного интерфейса (вопросы, help-сообщения, сводка) переведён на английский — примеры в этом разделе больше не на русском. `--libndk`/`ArmTranslator` как булев флаг заменён на `--arm-translator none|libndk|libhoudini` (см. раздел "Android: выбор ARM-транслятора" ниже) — то же самое значение теперь и в самом wizard, и в CLI-флагах, и в TOML (`arm_translator = "..."`, со старым `libndk = true/false` как deprecated-фоллбэком). Весь авто-детект хоста (GPU/дисплей/Venus/аудио/ARM-транслятор/`passt`) централизован в `andler-firmware::detect_all() -> HardwareDefaults` — wizard больше не содержит собственной детект-логики, только вызывает `detect_all()` один раз при старте и читает результат при построении дефолтов для каждого Advanced-вопроса.
...
Инстансы идентифицируются полным UUID (`InstanceId(Uuid)`, `core/andler-core/src/config/instance.rs`). Реализовано поведение, аналогичное `docker`/`docker-compose`:

- Команды, принимающие `<id>` (`start`, `stop`, `status`, `remove`, `snapshot`, `clone` и т.д.), принимают **префикс** UUID длиной от 1 и более символов вместо полного значения, без изменений на стороне CLI — сама CLI как передавала строку как есть, так и передаёт, разбор происходит на стороне daemon.
- Полный корректный UUID распознаётся как есть (быстрый путь, без обращения к реестру инстансов) — даже если такой инстанс ещё не зарегистрирован; ошибка "не найден" в этом случае приходит позже, из самой операции (`status`/`start`/...), как и раньше, а не из резолва ссылки.
- Если строка — не полный UUID, она ищется как **префикс** среди строковых представлений (`Uuid::to_string()`, стандартный дефисный формат, регистронезависимо) всех зарегистрированных `InstanceId`.
- Однозначное совпадение — команда выполняется как обычно.
- Несколько совпадений — команда завершается ошибкой `AmbiguousInstanceId` со списком совпавших полных ID, не выбирает случайный/первый. Это критично: неоднозначность при операциях с потенциально разрушительными последствиями (`remove --purge`) должна всегда требовать явного уточнения, не угадывания.
- Пустая строка — отдельная ошибка `EmptyInstanceRef` (не "0 совпадений с пустым префиксом", отдельное явное сообщение).
...
`Daemon::resolve_instance_id()` в `daemon/src/daemon/instance_ops.rs` (не в `query_ops`, как предполагалось изначально в этом разделе — сгруппировано рядом с `create_instance` в том же файле, а не по типу операции: единственный общий путь резолва, которым пользуются все `gRPC`-хендлеры). `daemon/src/service.rs` вызывает `resolve_instance_id()` вместо прежнего `andler_rpc::convert::parse_instance_id()` (который остаётся — это узкий парсер полного UUID без доступа к реестру, используется резолвером как быстрый путь) во всех хендлерах, принимающих `instance_id`/`source_instance_id` существующего инстанса. Резолв — один общий путь, не размазан по каждому отдельному CLI-обработчику: `daemon/src/service.rs` — единственное место, где `instance_id`-строка из запроса становится `InstanceId`, так что поведение (включая обработку неоднозначности) идентично для всех команд. `DaemonError::EmptyInstanceRef`/`InstanceRefNotFound`/`AmbiguousInstanceId` — новые варианты, замаплены на `Status::invalid_argument`/`Status::not_found`/`Status::invalid_argument` соответственно (`daemon/src/service.rs`, `impl From<DaemonError> for Status`). `andler list --full-id`/`-q` — новый флаг `Command::List`, `cli/src/status.rs::handle_list`.
...
- [x] **6. Wizard** (Высокий) — Интерактивный опрос (стрелки/Enter/Space через `inquire`), Basic/Advanced (выбор внутри wizard, не флаг), `--quick`. Готово — `cli/src/wizard/{mod,basic,advanced,summary}.rs`, `services/andler-firmware/src/detect/`. Не реализовано и сознательно не входит в текущий wizard: сетевой режим (всегда NAT) и лимит FPS — см. "Планируемые функции" в WIZARD.md. Автоматизированные end-to-end тесты полного интерактивного flow (не юнит-тесты на чистые функции) заблокированы тем, что `inquire::prompt_with_backend` — `pub(crate)`, недоступен из зависимого крейта; см. WIZARD.md, раздел "Integration tests".
- [x] **7. GPU render** (Высокий) — Venus (`virtio-gpu-gl`), VirGL, CPU; auto-detect минимальных версий хоста (kernel/QEMU/Mesa через `uname`/`qemu-system-x86_64 --version`/`glxinfo`). Готово — `services/andler-firmware/src/detect/gpu.rs`.
- [x] **8. Display** (Средний) — SDL/GTK с дефолтом по GPU-вендору (NVIDIA → SDL, AMD/Intel → GTK), Spice/None остаются ручным выбором. Готово — `services/andler-firmware/src/detect/gpu.rs` (тот же детект, что и для GPU render — общий источник вендора).
- [x] **9. Audio** (Средний) — virtio-sound/ich9-hda устройство (конфиг), auto-detect PipeWire/PulseAudio/None backend через сокеты сессии (`$XDG_RUNTIME_DIR`/`/run/user/{uid}`). Готово — `core/andler-core/src/config/audio.rs`, `services/andler-firmware/src/detect/audio.rs`. Проверка, что `virtio-sound` реально работает на хосте (не просто выбор backend'а) — не реализована.
- [x] **10. Input** (Средний) — virtio-tablet (абсолютные координаты) дефолтом уже был в `InputConfig::reference_default()`, mouse/USB — опционально в wizard. Связь с vdagent/дисплеем отдельно не реализована.
- [x] **11. Network** (Средний) — NAT с auto-detect `passt` (фоллбэк SLIRP). Готово — `services/andler-firmware/src/detect/network.rs`. Bridge, Isolated, port forwarding — не реализованы (сознательно вне текущего wizard, см. "Планируемые функции" в WIZARD.md).
- [ ] **12. CPU/Memory** (Средний) — Дефолты, balloon, zram — есть; связка `share=on` с Venus/KSM в самом `andler-qemu` (не только конфиг-поле `ksm`) — не проверена/не реализована.
- [ ] **13. Snapshots** (Средний) — Internal snapshots, CRUD, явная проверка `Running`/`Paused` — есть. Лимит количества снапшотов на инстанс — не реализован.
- [x] **14. Partial instance ID** (Средний) — Prefix UUID как Docker, неоднозначность — явная ошибка. Готово — `Daemon::resolve_instance_id()` (`daemon/src/daemon/instance_ops.rs`), `andler list --full-id`/`-q`.
- [ ] **15. Logging** (Средний) — `tracing_subscriber` + `EnvFilter` в `daemon/src/main.rs` есть; info-дефолт/debug/trace через явные CLI-флаги (не только `RUST_LOG`), отдельный лог QEMU от лога daemon — не реализованы.
- [ ] **16. NVIDIA metrics** (Средний) — Реализовано через CLI `nvidia-smi --format=csv` (`services/andler-firmware/src/metrics/gpu_nvidia.rs`), что уже даёт CSV-путь и толерантность к отсутствию `nvidia-smi`; NVML вместо парсинга текста — не реализован.
- [ ] **17. Boot priority** (Средний) — `bootindex` для диска/CD-ROM зашит (`disk=1`, `cd=2`, `backends/andler-qemu/src/cmdline.rs`); ручное переопределение пользователем — не реализовано.
- [x] **18. Android ARM-трансляторы** (Средний) — libndk/libhoudini — выбор одного активного по CPU-вендору, авто-детект + явный `--arm-translator`/wizard-вопрос. Готово — `core/andler-core/src/android_profile.rs`, `services/andler-firmware/src/detect/arm.rs`, `cli/src/wizard/advanced.rs`.
