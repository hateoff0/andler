...
- **Phase-exit gate, in addition to the per-commit gate**: before merging a phase branch into `main`, run the full E2E suite (not only unit + round-trip), re-check the phase's own guardrail items in the plan document, and update the plan's own phase-status marker in the merge commit. A bug the gate run itself uncovers (suite red for reasons unrelated to the phase's own diff) is fixed on the phase branch in its own commit — the alternative, "send it to `main` first", deadlocks: the phase cannot merge until the suite is green, and the suite is red because of that bug.
...
- **The instance supervisor's `run()` loop is intentionally sequential** (every current command is O(ms)); the first long-running `SupervisorCommand` (DiskChain ops, guest provisioning) MUST run in its own sub-task that acks through the command's oneshot — awaiting it inline freezes `status`/`cancel` behind it. The comment in `supervisor.rs` names this; a reviewer of any phase that adds a long op checks it there.
